    ...
    <div class="relative w-full bg-white p-2 group-hover:block lg:min-w-[300px] lg:rounded-sm">
        @if ($cart->itemsCount > 0)
            ...
            <div class="w-full text-center mt-2 flex justify-between text-sm" v-if="hasItems">
                <a href="{{ route('order.create') }}" class="rounded-sm text-white bg-green-600 px-4 py-1 active:shadow-inner hover:bg-green-500">
                    Go to checkout
                </a>
        
                <a href="{{ route('cart.index') }}" class="rounded-sm text-white bg-sky-600 px-4 py-1 active:shadow-innerhover:bg-sky-500">
                    View Cart
                </a>
            </div>
        @else
