<template>
  <ViewContainer>
    ...
    <template slot="header"> Products List </template>
      ...
      <Card v-for="product in products" :key="product.id">
        <router-link :to="{ name: 'Product', params: { id: product.id } }">
          ...
          >
            ...
            <div>
              <div class="text-xs font-semibold">#{{ product.id }}</div>
              <div class="mt-2">
...
export default {
  ...
  mounted() {
    Object.keys(this.$route.query).forEach((key) => {
      ...
      if (!_isEmpty(this.$route.query.subCategories)) {
        this.filterData.subCategories = [];
        this.filterData.subCategories.push(...this.$route.query.subCategories);
      }
    });
  },
