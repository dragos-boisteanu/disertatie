	...
	<ViewContainer>
        ...
        "
            ...
            "
						...
						>
							...
							<Input
								...
								:class="{
									...
									'border-green-600': $v.order.phoneNumber.$dirty && !$v.order.phoneNumber.$error,
								}"
								@blur.native="getClient"
							></Input>
						</InputGroup>

						<InputGroup id="clientEmail" label="Email" :hasError="$v.order.email.$error" :eclass="{ 'flex-1': true }">
							<template v-slot:errors>
							...
							<Input
								...
								:class="{
									...
									'border-green-600': $v.order.email.$dirty && !$v.order.email.$error,
								}"
							></Input>
						</InputGroup>
					</div>
					...
					>
						...
						<Select
							...
							:class="{
								...
								'border-green-600': $v.order.deliveryMethodId.$dirty && !$v.order.deliveryMethodId.$error,
							}"
						>
							<option value="" selected disabled>Select delivery method</option>
							<option v-for="method in getDeliveryMethods" :key="method.id" :value="method.id">
					...
					<div class="flex flex-col gap-2 md:flex-row md:items-center" v-if="isTableOrder || isDeliveryOrder">
						<div class="w-full" v-if="isDeliveryOrder">
							<InputGroup
								...
								:eclass="{ 'flex-1': true }"
							>
								<Select v-model="order.address" id="clientAddresses" name="clientAddresses">
									<option value="" selected disabled>
										Select client address
									</option>
									<option v-for="(address, index) in client.addresses" :key="index" :value="address.address">
										{{ address.address }}
									</option>
								</Select>
							</InputGroup>
							...
							>
								...
								<Input
									...
									:class="{
										...
										'border-green-600': $v.order.address.$dirty && !$v.order.address.$error,
									}"
						...
						>
							...
							<Select
								...
								:class="{
									...
									'border-green-600': $v.order.tableId.$dirty && !$v.order.tableId.$error,
								}"
							...
							>
								...
								<option
									...
									:key="table.id"
									:value="table.id"
									:disabled="table.status.name === 'Busy' || table.status.name === 'Disabled'"
								>
									{{ table.name }}
								</option>
							</Select>
					...
					<InputGroup id="observations" label="Observations" :hasError="$v.order.observations.$error">
						<template v-slot:errors>
						...
						<Textarea
							...
							:class="{
								...
								'border-green-600': $v.order.observations.$dirty && !$v.order.observations.$error,
							}"
...
export default {
	...
	methods: {
		...
		addProductToOrder(product) {
			...
			if (existingProduct) {
				...
				if (newQuantity <= existingProduct.stock) {
					...
					this.$toast.error("Product total quantity is greater than the available stock");
				}
			} else {
		...
		resetForm() {
			...
			(this.selectedItemId = ""), (this.selectedItemQuantity = ""), (this.showAddProductModalState = false);
		},
	},
