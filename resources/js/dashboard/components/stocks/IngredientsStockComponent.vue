<template>
  <div class="w-full">
    ...
    >
      ...
      >
        <template v-slot:errors>
          <p v-if="!$v.filter.id.integer">The id field must be an integer</p>
          <p v-if="!$v.filter.id.minValue">The id filed must be atleast 1</p>
        </template>
        <Input
          v-model="filter.id"
          id="id"
          name="id"
          :class="{
            'border-red-600': $v.filter.id.$error,
            'border-green-600': $v.filter.id.$dirty && !$v.filter.id.$error,
          }"
      ...
      >
        ...
        <Input
          ...
          name="name"
          :eclass="{
            'border-red-600': $v.filter.name.$error,
            'border-green-600': $v.filter.name.$dirty && !$v.filter.name.$error,
          }"
          :disabled="searching || updating"
    ...
    <div
      ...
      class="mt-4 flex flex-col bg-white shadow rounded-sm p-5 md:flex-1"
    >
      ...
      <form>
        <div class="flex gap-2">
          <InputGroup id="stock" label="Stock" :eclass="{ 'flex-1': true }">
            <Input
          ...
          <InputGroup id="unit" label="unit" :eclass="{ 'flex-1': true }">
            <Input
...
export default {
  async mounted() {
    if (
      this.getIngredientStockDetails &&
      (this.getIngredientStockDetails.id === this.$route.params.id ||
        this.getIngredientStockDetails.name === this.$route.params.name)
    ) {
