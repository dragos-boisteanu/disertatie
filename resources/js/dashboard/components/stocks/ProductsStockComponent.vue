  ...
  <div class="w-full">
      ...
      >
        ...
        >
          <template v-slot:errors>
            <p v-if="!$v.barcode.required">The barcode field is required</p>
          </template>
          <Input
            v-model="barcode"
            id="barcode"
            name="barcode"
            :class="{
              'border-red-600': $v.barcode.$error,
              'border-green-600': $v.barcode.$dirty && !$v.barcode.$error,
            }"
            :disabled="searching"
    ...
    >
      <h2 class="text-xl font-semibold my-2">
        <span>{{ getProductStockDetails.name }}</span>
        <span>{{ getProductStockDetails.weight }}</span>
        <span>{{ getProductStockDetails.unit }}</span>
      </h2>
      ...
      <form>
        ...
        <div v-if="hasIngredients" class="mt-4">
          <h3 class="font-semibold text-lg my-2">Ingredients</h3>
          <ul>
            <li
              v-for="ingredient in getProductStockDetails.ingredients"
              :key="ingredient.id"
            >
...
export default {
  ...
  computed: {
    ...
    hasIngredients() {
      return (
        Boolean(this.getProductStockDetails.hasIngredients) &&
        this.getProductStockDetails.ingredients.length > 0
      );
    },
